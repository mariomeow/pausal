...
function getDatabaseMap(): databaseType {
    ...
    if (!localStorage.getItem("pausalTuristickeZajedniceInfo")) setPausalOption("turisticke_zajednice_info", false, { skupina: "prva", promet: 0, godine: [myDate.getFullYear()], placeneGodine: [] })

    const pausalInfoLocal = JSON.parse(localStorage.getItem("pausalTuristickeZajedniceInfo")!)

    if (!pausalInfoLocal.godine.includes(myDate.getFullYear())) localStorage.setItem("pausalTuristickeZajedniceInfo", JSON.stringify({
        ...pausalInfoLocal,
...
export function setPausalOption(option: "poslodavac" | "komorski_doprinos" | "turisticke_zajednice" | "turisticke_zajednice_info", value: boolean, objekt?: { skupina: "prva" | "druga" | "treca" | "cetvrta" | "peta", promet: number, godine: number[], placeneGodine: number[] }) {
...
function getPausalOption(option: "poslodavac" | "komorski_doprinos" | "turisticke_zajednice" | "turisticke_zajednice_info") {
    ...
    if (option == "turisticke_zajednice_info") return JSON.parse(localStorage.getItem("pausalTuristickeZajedniceInfo")!)
}

export function toggleTuristickaGodina(year: number) {
    if (pausalInfo.turisticke_zajednice_info.placeneGodine.includes(year)) {
        pausalInfo.turisticke_zajednice_info.placeneGodine = pausalInfo.turisticke_zajednice_info.placeneGodine.filter((item: number) => item != year)
    } else {
        pausalInfo.turisticke_zajednice_info.placeneGodine.push(year)
    }

    localStorage.setItem("pausalTuristickeZajedniceInfo", JSON.stringify(pausalInfo.turisticke_zajednice_info))
}
